...
module Setup
  class OmnitruckClient
    ...
    def initialize(options)
      @platform = options[:platform_family] == "rhel" ? "el" : options[:platform]
      @platform_version = options[:platform_family] == "rhel" ? options[:platform_version].to_i : options[:platform_version]
      @machine_architecture = options[:arch] || 'x86_64'
    end
